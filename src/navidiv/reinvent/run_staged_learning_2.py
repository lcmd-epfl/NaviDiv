...
def create_packages(
    ...
    for stage in stages:
        ...
        try:
            terminator: terminator_callable = getattr(
                terminators, f"{terminator_name}Terminator"
            )
        except KeyError:
        ...
        if stage.diversity_filter:
            diversity_filter = setup_diversity_filter(
                stage.diversity_filter, rdkit_smiles_flags
            )
...
def run_staged_learning(
    ...
    diversity_scorer: list[dict] = None,
    *args,
    **kwargs,
):
    ...
    avoid_model_filenames = eval(parameters.avoid_file)
    logger.info(
        f"Using prior model {prior_model_filename}, agent model {agent_model_filename}"
    )
    logger.info(f"Using avoid model(s) {avoid_model_filenames}")
    ...
    prior = adapter
    avoids = []
    for avoid_model_filename in avoid_model_filenames:
        adapter_avoid, _, _ = create_adapter(avoid_model_filename, "inference", device)
        avoid = adapter_avoid
        disable_gradients(avoid)
        avoids.append(avoid)
    ...
    if parameters.smiles_file:
        ...
        logger.info(
            f"Input molecules/fragments read from file {parameters.smiles_file}"
        )
    ...
    if parameters.use_checkpoint and "staged_learning" in agent_save_dict:
        ...
        diversity_filter = setup_diversity_filter(
            config.diversity_filter, rdkit_smiles_flags2
        )
    ...
    with Handler() as handler:
        for run, package in enumerate(packages):
            ...
            if global_df_only:  # global DF always overwrites stage DFs
                ...
                logger.debug("Using global DF")
            else:
                state = ModelState(agent, package.diversity_filter)
                logger.debug("Using stage DF")
            ...
            optimize = model_learning(
                ...
                tb_logdir=logdir,
                avoid=avoids,
                scorer_dicts=diversity_scorer["scorer_dicts"],
            )
